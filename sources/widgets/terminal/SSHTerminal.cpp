...
SSHTerminal::SSHTerminal(const Session &session, QWidget *parent) : BaseTerminal(parent) {
    startShellProgram();
    _session = session;
}
...
QString SSHTerminal::createShellFile()
{
    ...
    // 用远程管理数据替换文件内的关键字
    fileString.replace("<<USER>>", _session.userName.c_str());
    fileString.replace("<<SERVER>>", _session.host.c_str());
    fileString.replace("<<PORT>>", QString::number(_session.port));
    // 根据是否有证书，替换关键字
    if (_session.authType == SSH_AUTH_TYPE::PASSWD) {
        ...
        fileString.replace("<<PRIVATE_KEY>>", QString("-i " + QString(_session.keyFile.c_str())));
