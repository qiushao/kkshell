...
EditSessionDialog::EditSessionDialog(const std::string &sessionType, QWidget *parent): QDialog(parent) {
...
void EditSessionDialog::initViews(const std::string &sessionType) {
...
void EditSessionDialog::newSerialSessionLayout() {
...
void EditSessionDialog::newSSHSessionLayout() {
    ...
    QObject::connect(sshAuthTypeEdit, &QComboBox::currentTextChanged, this, &EditSessionDialog::onSSHAuthTypeChanged);
}

void EditSessionDialog::newButtonLayout() {
    ...
    QObject::connect(applyButton, &QPushButton::clicked, this, &EditSessionDialog::onApplyButtonClicked);
    QObject::connect(cancelButton, &QPushButton::clicked, this, &EditSessionDialog::onCancelButtonClicked);
}

void EditSessionDialog::onApplyButtonClicked() {
...
void EditSessionDialog::onCancelButtonClicked() {
    hide();
}

void EditSessionDialog::saveSession() {
    ConfigManager *conf = ConfigManager::getInstance();
    std::string sessionName = sessionNameEidt->text().toStdString();
...
void EditSessionDialog::onSSHAuthTypeChanged(const QString &authType) {
    if (authType == "passwd") {
        sshKeyFileEdit->setEnabled(false);
...
void EditSessionDialog::editSession(const std::string &sessionName, const std::string &sessionType) {
